...
class DtoDataProcessAmazon:
    ...
    def replace_titles(self, territory_filter='US', sku_number_filter=' ', title_column='NEW_TITLE'):
        try:
            top_english_titles = self.df[(self.df[self.TERRITORY] == territory_filter) & (self.df[self.SKU_NUMBER] != sku_number_filter)]
            most_frequent_titles = top_english_titles.groupby(self.SKU_NUMBER)[title_column].apply(lambda x: x.mode().iloc[0] if len(x.mode()) > 0 else x.iloc[0]).reset_index(name=title_column)
            self.df = pd.merge(self.df, most_frequent_titles, on=self.SKU_NUMBER, how='left')
            ...
            print(f"Error replacing titles - KeyError: {e}")
        except Exception as e:
